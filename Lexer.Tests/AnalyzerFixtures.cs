...
public class AnalyzerFixtures
{
    ...
    [InlineData("""
        ...
        """)]
    private async Task GivenCode_WhenAnalyzeWithSomeDisabledRules_ThenResultDoesNotContainDisabledTypeOfLexemes(string str)
    {
        // Arrange
        RuleSet rules = new(new List<IRule>() {
            new WordsRule(new[] {"public", "private", "class", "void" }, "Keyword", isEnabled: false),
            new RegexRule(new(@"\b[_A-Za-z][_A-Za-z0-9]*\b"), "Name"),
            new RegexRule(new(@"\b\d+\b"), "IntNumber"),
            new RegexRule(new(@"[\(\)\{\}\.;]"), "Separator"),
            new RegexRule(new(@"[-+/*=]"), "Operator"),
        ...
        // Assert
        result.Lexemes.Where(l => l.Type == "Keyword").Any().Should().BeFalse();
    }
}
